...
class BancoDados {
    ...
    static setLocalStorage() {
        ...
        localStorage.setItem('db_app_cadastro_react', dbFormatado)
    }

    // MÉTODO PARA RECUPERAR DADOS DO LOCAL LOCALSTORAGE
    static getLocalStorage() {
        // RECUPERAR OS DADOS ARMAZENADO, UTILIZANDO O NOME DA KEY
        const dbJson = localStorage.getItem('db_app_cadastro_react')
    ...
    static delete(rota, id) {
        this.db[rota] = this.db[rota].filter(item => item.id !== id)
        this.setLocalStorage()
    }

    // MÉTODO PARA ATUALIZAR ITEM
    static update(rota, id, updatedData) {
        const index = this.db[rota].findIndex(item => item.id === id)
        if (index !== -1) {
            this.db[rota][index] = { ...this.db[rota][index], ...updatedData }
            this.setLocalStorage()
            return this.db[rota][index]
        }
        return null
    }
}
